...
const findMatchRecordById = (matches: MatchT[], targetMatchId: number) => matches.find(({ id }) => id === targetMatchId);
        ...
        <div key={si} className={styles.stage}>
          ...
          <div>
            {Array.isArray(stage) && stage?.map((bracketUnit, mi) => {
              ...
              // ------------------------------------------------------------------------------
              const matchRecord = bracketUnit.matchId ? findMatchRecordById(matches, bracketUnit.matchId) as MatchT : null;
                ...
                >
                  {matchRecord && (
                    <>
                      ...
                      <div>
                        <div className={cl(styles.playersPair, matchRecord.score && (matchRecord.player1_id === parseInt(matchRecord?.winner_id as any, 10)) ? styles.winner : styles.loser)}>
                          <span className={styles.player}>{playersNames.get(matchRecord.player1_id as number)}</span>
                          <span className={styles.player}>{isDoubles && playersNames.get(matchRecord.player3_id as number)}</span>
                        </div>
                        <p className={styles.vs}>VS</p>
                        <div className={cl(styles.playersPair, matchRecord.score && (matchRecord.player2_id === parseInt(matchRecord?.winner_id as any, 10)) ? styles.winner : styles.loser)}>
                          <span className={styles.player}>{playersNames.get(matchRecord.player2_id as number)}</span>
                          <span className={styles.player}>{isDoubles && playersNames.get(matchRecord.player4_id as number)}</span>
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {matchRecord ?
                    ...
                    <button
                      className={cl(styles.drawUnitAction, styles.createMatch)}
                      disabled={isDisabled}
